...
(function () {
    ...
    drawTurbos(ctx, mode, liveTurbos, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const t of turbosToDraw) {
          ...
          // Balanced flame layers (compact but visible burst)
          const flameCount = 4;
          for (let i = 0; i < flameCount; i++) {
            const flamePhase = (currentTime * 0.006 + i * Math.PI / 3) % (Math.PI * 2);
            const flameRadius = t.r + 2 + i * 1.5 + Math.sin(flamePhase) * 2;
            const flameAlpha = (Math.sin(flamePhase) * 0.4 + 0.6) * (1 - i * 0.18);
          ...
          // Balanced fire particles (visible but not oversized)
          const particleCount = 10;
          for (let i = 0; i < particleCount; i++) {
            const angle = (i / particleCount) * Math.PI * 2 + currentTime * 0.01;
            const particleRadius = t.r + 5 + Math.sin(currentTime * 0.012 + i) * 2;
    ...
    drawTeleports(ctx, mode, liveTeleports, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const tp of teleportsToDraw) {
          ...
          // Balanced portal rings (compact but visible burst)
          const portalCount = 5;
          for (let i = 0; i < portalCount; i++) {
            const portalPhase = (currentTime * 0.005 + i * Math.PI / 3) % (Math.PI * 2);
            const portalRadius = tp.r + 2 + i * 1.5 + Math.sin(portalPhase) * 1.5;
            const portalAlpha = (Math.sin(portalPhase + i) * 0.3 + 0.4) * (1 - i * 0.15);
          ...
          // Balanced dimensional particles (visible but not oversized)
          const particleCount = 8;
          for (let i = 0; i < particleCount; i++) {
            const angle = (i / particleCount) * Math.PI * 2 + currentTime * 0.004;
            const particleRadius = tp.r + 4 + Math.sin(currentTime * 0.008 + i) * 2;
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const tpGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'P' : '🌀';
    ...
    drawPoisons(ctx, mode, livePoisons, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const poison of poisonsToDraw) {
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const poisonGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'P' : '☠️';
    ...
    drawMagnets(ctx, mode, liveMagnets, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const mg of magnetsToDraw) {
          ...
          // DEBUG: Compact magnet rendering active
          if (performance.now() % 5000 < 100) console.log('✅ Balanced magnet active:', mg.x, mg.y, 'fieldRadius max:', mg.r + 12);
          
          // Balanced magnetic field lines (compact but visible burst)
          const fieldCount = 6;
          for (let i = 0; i < fieldCount; i++) {
            const fieldPhase = (currentTime * 0.002 + i * Math.PI / 4) % (Math.PI * 2);
            const fieldRadius = mg.r + 3 + i * 1.5 + Math.sin(fieldPhase) * 2;
            const fieldAlpha = (Math.sin(fieldPhase + i) * 0.3 + 0.4) * (1 - i * 0.12);
          ...
          // Balanced magnetic particles (visible but not oversized)
          const particleCount = 10;
          for (let i = 0; i < particleCount; i++) {
            const angle = (i / particleCount) * Math.PI * 2 + currentTime * 0.004;
            const particleRadius = mg.r + 5 + Math.sin(currentTime * 0.008 + i) * 2;
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const mgGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'M' : '🧲';
    ...
    drawTimeFreezes(ctx, mode, liveTimeFreezes, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const tf of timeFreezesToDraw) {
          ...
          // Balanced ice crystal rings (compact but visible burst)
          const crystalCount = 5;
          for (let i = 0; i < crystalCount; i++) {
            const crystalPhase = (currentTime * 0.001 + i * Math.PI / 3) % (Math.PI * 2);
            const crystalRadius = tf.r + 2 + i * 2 + Math.sin(crystalPhase) * 1.5;
            const crystalAlpha = (Math.sin(crystalPhase + i) * 0.2 + 0.4) * (1 - i * 0.12);
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const tfGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'F' : '❄️';
    ...
    drawIceFreezers(ctx, mode, liveIceFreezers, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const ice of iceFreezersToDraw) {
          ...
          // Compact shield-like aura
          try {
            drawGlow(ctx, ice.x, ice.y, ice.r * 1.3 * pulse, '#b3e5fc', glowAlpha(0.15 * pulse, 0.8, 0.3));
            drawGlow(ctx, ice.x, ice.y, ice.r * 1.1 * pulse, '#81d4fa', glowAlpha(0.3 * pulse, 1.0, 0.4));
            drawGlow(ctx, ice.x, ice.y, ice.r * 0.9 * fastPulse, '#e1f5fe', glowAlpha(0.5 * fastPulse, 1.3, 0.6));
          } catch {}
          
          // Balanced ice crystal rings (compact burst)
          const ringCount = 3;
          for (let ring = 0; ring < ringCount; ring++) {
            const ringRadius = ice.r + 4 + ring * 3;
          ...
          if (mode === 'editor' && mapDef?.editorVectorIconsOnly) {
            ...
            const icon = (mode === 'editor' && mapDef?.editorEmojiEnabled === false) ? 'ICE' : '🧊';
  ...
  drawTestpowers(ctx, mode, liveTestpowers, mapDef, drawGlow, glowAlpha) {
    ...
    drawGhosts(ctx, mode, liveGhosts, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const g of ghostsToDraw) {
          ...
          // Balanced spectral aura rings (compact but visible burst)
          const auraCount = 4;
          for (let i = 0; i < auraCount; i++) {
            const auraPhase = (currentTime * 0.001 + i * Math.PI / 3) % (Math.PI * 2);
            const auraRadius = g.r + 2 + i * 2.5 + Math.sin(auraPhase) * 2;
            const auraAlpha = (Math.sin(auraPhase + i) * 0.15 + 0.2) * (1 - i * 0.15) * float;
          ...
          // Balanced spectral wisps (visible but not oversized)
          const wispCount = 8;
          for (let i = 0; i < wispCount; i++) {
            const angle = (i / wispCount) * Math.PI * 2 + currentTime * 0.003;
            const wispRadius = g.r + 4 + Math.sin(currentTime * 0.004 + i) * 3;
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const gGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'G' : '👻';
      ...
      try {
        ...
        for (const r of ramsToDraw) {
          ...
          if (mode === 'editor' && mapDef?.editorVectorIconsOnly) { 
            ...
            const rGlyph = (mode === 'editor' && mapDef?.editorEmojiEnabled === false) ? 'RAM' : '💥';
    ...
    drawSpinners(ctx, mode, mapDef, liveSpinners) {
      ...
      try {
        // Determine which spinners to render based on mode and override flags
        let spinnersToDraw = [];
        
        // Nuclear fix: skip all static spinners if override flags are set
        if ((mode === 'play' || mode === 'race') && 
            (window._skipStaticSpinners || window._forceOnlyLiveSpinners || window._hideStaticSpinners)) {
          // In play mode with override flags: render ONLY liveSpinners, completely ignore mapDef.spinners
          spinnersToDraw = liveSpinners || [];
          console.log('🔄 Spinner nuclear fix active - rendering', spinnersToDraw.length, 'live spinners only, ignoring', (mapDef?.spinners || []).length, 'static');
        } else {
          // Normal logic for non-override cases
          ...
          spinnersToDraw = (mode === 'play' || mode === 'race')
            ? (liveSpinners || [])
            : (mapDef?.spinners || []);
        }
    ...
    drawFrame(ctx, canvas, env) {
      ...
      // 3) Spinners - SKIP in play mode to prevent duplicates (handled by main render loop)
      try {
        if (mode !== 'play' && mode !== 'race') {
    ...
    drawLightnings(ctx, mode, liveLightnings, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const item of lightningsToDraw) {
          ...
          // Compact shield-like glow
          try {
            const glowIntensity = pulse * 1.5;
            drawGlow(ctx, x, y, dynamicRadius + 6, '#FFD700', glowAlpha(0.4 * glowIntensity, 1.8 * glowIntensity, 1.2));
            
            // Secondary glow ring
            drawGlow(ctx, x, y, dynamicRadius + 3, '#FFFFFF', glowAlpha(0.2 * pulse, 0.8 * pulse, 0.6));
          } catch {}
      ...
      try {
        ...
        for (const item of warpzonesToDraw) {
          ...
          try {
            ...
            // Inner intense glow
            drawGlow(ctx, item.x, item.y, animatedRadius * 1.0, '#FFFFFF', glowAlpha(0.3 * ultraPulse, 0.8 * ultraPulse, 0.2));
          } catch {}
      ...
      try {
        ...
        for (const item of quantumdashsToDraw) {
          ...
          try {
            ...
            // Core quantum field
            drawGlow(ctx, item.x, item.y, quantumRadius * 1.1, '#81D4FA', glowAlpha(0.4 * fieldDistortion, 0.7 * fieldDistortion, 0.18));
            // Crystal core
    ...
    drawNebulas(ctx, mode, liveNebulas, mapDef, drawGlow, glowAlpha) {
      ...
      try {
        ...
        for (const item of nebulasToDraw) {
          ...
          try {
            drawGlow(ctx, item.x, item.y, animatedRadius * 1.6, '#4A0E4E', glowAlpha(0.1 * starPulse, 0.25 * starPulse, 0.08));
            drawGlow(ctx, item.x, item.y, animatedRadius * 1.3, '#4A0E4E', glowAlpha(0.2 * nebulaPhase, 0.4 * nebulaPhase, 0.12));
            drawGlow(ctx, item.x, item.y, animatedRadius * 1.1, '#FFFFFF', glowAlpha(0.3 * starPulse, 0.6 * starPulse, 0.15));
          } catch {}
          ...
          if (!(mode === 'editor' && window.editorVectorIconsOnly)) {
            const glyph = (mode === 'editor' && !window.editorEmojiEnabled) ? 'N' : '🔹';
