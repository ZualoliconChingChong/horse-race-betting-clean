...
(function () {
  ...
  const moduleObj = {
    /** Vẽ icon vector (dành cho editor) */
    _drawVectorIcon(ctx, type, x, y, r){
      try {
        ...
        // Nội dung icon đơn giản, rõ ràng
        if (type === 'boost' || type === 'turbo'){
          ...
          ctx.strokeStyle='rgba(0,0,0,0.7)';
          const L=r*0.6;
          ctx.beginPath(); ctx.moveTo(-L,-L); ctx.lineTo(L,L); ctx.stroke();
          ctx.beginPath(); ctx.moveTo(L,-L); ctx.lineTo(-L,L); ctx.stroke();
        } else if (type === 'ram'){
          // Sao nổ
          ...
          for (let i=0;i<4;i++){
            ctx.beginPath();
            const rr = r*0.2 + i*r*0.15;
            ctx.arc(0, 0, rr, 0, Math.PI*1.8);
            ctx.stroke();
          }
          // Tâm tornado
    ...
    /**
     * Vẽ Boosts (sao chép nguyên xi từ index.html). An toàn: tự save/restore.
    ...
    drawBoosts(ctx, mode, liveBoosts, mapDef, drawGlow, glowAlpha) {
      try {
        ctx.save();
        // Fix: In play mode, render both static boosts from mapDef and live boosts
        const boostsToDraw = (mode === 'play' || mode === 'race')
          ? [ ...(mapDef?.boosts || []), ...(liveBoosts || []) ]
          : [ ...(mapDef?.boosts || []), ...((Array.isArray(liveBoosts) ? liveBoosts : [])) ];
        ...
        for (const b of boostsToDraw) {
          ...
          // Icon with shadow
          ctx.font = `bold ${Math.max(12, Math.round(b.r * 0.95))}px "Segoe UI Emoji","Segoe UI Symbol",system-ui,sans-serif`;
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const boostGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'B' : '⚡';
    ...
    drawTurbos(ctx, mode, liveTurbos, mapDef, drawGlow, glowAlpha) {
      try {
        ctx.save();
        // Fix: In play mode, render both static turbos from mapDef and live turbos
        const turbosToDraw = (mode === 'play' || mode === 'race')
          ? [ ...(mapDef?.turbos || []), ...(liveTurbos || []) ]
          : [ ...(mapDef?.turbos || []), ...((Array.isArray(liveTurbos) ? liveTurbos : [])) ];
        const currentTime = performance.now();
        ...
        for (const t of turbosToDraw) {
          ...
          // Icon with fire glow
          ctx.font = `bold ${Math.max(12, Math.round(t.r * 0.9))}px "Segoe UI Emoji","Segoe UI Symbol",system-ui,sans-serif`;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const turboGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'T' : '🚀';
    ...
    try {
      ...
      for (const item of itemsToDraw) {
        if (item.consumed && (mode === 'play' || mode === 'race')) continue;
                ...
                // Compact spiral portal effect
        const pulseSpeed = 0.004;
        const pulse = Math.sin(currentTime * pulseSpeed) * 0.2 + 0.8;
        
        // Small glow like shield
        try {
          drawGlow(ctx, item.x, item.y, 18 * pulse, '#9C27B0', glowAlpha(0.4 * pulse, 0.8 * pulse, 0.2));
        } catch {}
        
        // Compact spiral rings (2 rings only)
        for (let i = 0; i < 2; i++) {
          const ringPhase = (currentTime * 0.003 + i * Math.PI) % (Math.PI * 2);
          const ringRadius = 12 + 4 + i * 3 + Math.sin(ringPhase) * 2;
          const ringAlpha = (Math.sin(ringPhase) * 0.4 + 0.6) * 0.5;
          
          ctx.strokeStyle = `rgba(156, 39, 176, ${ringAlpha})`;
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          ctx.arc(item.x, item.y, ringRadius, 0, Math.PI * 1.5);
          ctx.stroke();
        }
        
        // Compact main circle (same size as shield)
        const gradient = ctx.createRadialGradient(
          item.x - 12 * 0.3, item.y - 12 * 0.3, 0,
          item.x, item.y, 12
        );
        gradient.addColorStop(0, '#E1BEE7');
        gradient.addColorStop(0.7, '#9C27B0');
        ...
        // Compact border (thin like shield)
        ...
        // Icon
        ctx.font = `bold ${Math.max(12, Math.round(12 * 0.85))}px ${EMOJI_FONT}`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillStyle = 'white';
        if (mode === 'editor' && mapDef?.editorVectorIconsOnly) {
          try { this._drawVectorIcon(ctx, 'teleport', item.x, item.y, 12); } catch {}
        } else {
          const configEmoji = window.config?.powerUps?.teleport?.emoji;
    ...
    try {
      ...
      for (const item of itemsToDraw) {
        if (item.consumed && (mode === 'play' || mode === 'race')) continue;
                ...
                // Compact toxic cloud
        const toxicPulse = Math.sin(currentTime * 0.004) * 0.3 + 0.7;
        
        // Small toxic glow
        try {
          drawGlow(ctx, item.x, item.y, 16 * toxicPulse, '#4CAF50', glowAlpha(0.4 * toxicPulse, 0.7 * toxicPulse, 0.3));
        } catch {}
        
        // Compact toxic bubbles (4 bubbles only)
        for (let i = 0; i < 4; i++) {
          const angle = (i * Math.PI / 2) + (currentTime * 0.003);
          const bubbleRadius = 12 + 3 + Math.sin(currentTime * 0.007 + i) * 2;
          const bx = item.x + Math.cos(angle) * bubbleRadius;
          const by = item.y + Math.sin(angle) * bubbleRadius;
          
          ctx.fillStyle = `rgba(76, 175, 80, ${0.6 * toxicPulse})`;
          ctx.beginPath();
        ...
        ctx.fillStyle = 'white';
        if (mode === 'editor' && mapDef?.editorVectorIconsOnly) {
          try { this._drawVectorIcon(ctx, 'poison', item.x, item.y, 12); } catch {}
        } else {
          const configEmoji = window.config?.powerUps?.poison?.emoji;
          const icon = (mode === 'editor' && mapDef?.editorEmojiEnabled === false) 
            ? 'TOX' 
            : (configEmoji || '☠️');
          ctx.fillText(icon, item.x, item.y);
    ...
    try {
      ...
      for (const item of itemsToDraw) {
        if (item.consumed && (mode === 'play' || mode === 'race')) continue;
                ...
                // Compact magnetic field
        const magnetPulse = Math.sin(currentTime * 0.005) * 0.3 + 0.7;
        
        // Small magnetic glow
        try {
          drawGlow(ctx, item.x, item.y, 16 * magnetPulse, '#FF5722', glowAlpha(0.5 * magnetPulse, 0.9 * magnetPulse, 0.3));
        } catch {}
        
        // Compact magnetic lines (4 lines only)
        for (let i = 0; i < 4; i++) {
          const angle = (i * Math.PI / 2) + (currentTime * 0.002);
          const lineRadius = 12 + 3 + Math.sin(currentTime * 0.008 + i) * 2;
          const x1 = item.x + Math.cos(angle) * lineRadius;
          const y1 = item.y + Math.sin(angle) * lineRadius;
          const x2 = item.x + Math.cos(angle + Math.PI) * lineRadius;
          const y2 = item.y + Math.sin(angle + Math.PI) * lineRadius;
          
          ctx.strokeStyle = `rgba(255, 87, 34, ${0.6 * magnetPulse})`;
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          ctx.moveTo(x1, y1);
          ctx.lineTo(x2, y2);
        ...
        ctx.fillStyle = 'white';
        if (mode === 'editor' && mapDef?.editorVectorIconsOnly) {
          try { this._drawVectorIcon(ctx, 'magnet', item.x, item.y, 12); } catch {}
        } else {
          const configEmoji = window.config?.powerUps?.magnet?.emoji;
          const icon = (mode === 'editor' && mapDef?.editorEmojiEnabled === false) 
            ? 'MAG' 
            : (configEmoji || '🧲');
          ctx.fillText(icon, item.x, item.y);
    ...
    try {
      ...
      for (const item of itemsToDraw) {
        if (item.consumed && (mode === 'play' || mode === 'race')) continue;
                ...
                // Compact ice wave
        const icePulse = Math.sin(currentTime * 0.003) * 0.25 + 0.75;
        
        // Small ice glow
        try {
          drawGlow(ctx, item.x, item.y, 18 * icePulse, '#03A9F4', glowAlpha(0.4 * icePulse, 0.8 * icePulse, 0.4));
        } catch {}
        
        // Compact ice crystals (6 crystals only)
        for (let i = 0; i < 6; i++) {
          const angle = (i * Math.PI / 3) + (currentTime * 0.001);
          const crystalRadius = 12 + 4 + Math.sin(currentTime * 0.006 + i) * 1.5;
          const cx = item.x + Math.cos(angle) * crystalRadius;
          const cy = item.y + Math.sin(angle) * crystalRadius;
          
          ctx.fillStyle = `rgba(179, 229, 252, ${0.8 * icePulse})`;
          ctx.beginPath();
        ...
        ctx.fillStyle = 'white';
        if (mode === 'editor' && mapDef?.editorVectorIconsOnly) {
          try { this._drawVectorIcon(ctx, 'timeFreeze', item.x, item.y, 12); } catch {}
        } else {
          const configEmoji = window.config?.powerUps?.timeFreeze?.emoji;
          const icon = (mode === 'editor' && mapDef?.editorEmojiEnabled === false) 
            ? 'ICE' 
            : (configEmoji || '❄️');
          ctx.fillText(icon, item.x, item.y);
    ...
    try {
      ...
      for (const item of itemsToDraw) {
        if (item.consumed && (mode === 'play' || mode === 'race')) continue;
                ...
                // Compact spirit aura
        const spiritPulse = Math.sin(currentTime * 0.002) * 0.4 + 0.6;
        
        // Small spirit glow
        try {
          drawGlow(ctx, item.x, item.y, 20 * spiritPulse, '#9E9E9E', glowAlpha(0.3 * spiritPulse, 0.6 * spiritPulse, 0.2));
        } catch {}
        
        // Compact spirit wisps (3 wisps only)
        for (let i = 0; i < 3; i++) {
          const angle = (i * Math.PI * 2 / 3) + (currentTime * 0.001);
          const wispRadius = 12 + 5 + Math.sin(currentTime * 0.005 + i) * 3;
          const wx = item.x + Math.cos(angle) * wispRadius;
          const wy = item.y + Math.sin(angle) * wispRadius;
          
          ctx.fillStyle = `rgba(224, 224, 224, ${0.7 * spiritPulse})`;
          ctx.beginPath();
        ...
        ctx.fillStyle = 'white';
        if (mode === 'editor' && mapDef?.editorVectorIconsOnly) {
          try { this._drawVectorIcon(ctx, 'ghost', item.x, item.y, 12); } catch {}
        } else {
          const configEmoji = window.config?.powerUps?.ghost?.emoji;
          const icon = (mode === 'editor' && mapDef?.editorEmojiEnabled === false) 
            ? 'GHO' 
            : (configEmoji || '👻');
          ctx.fillText(icon, item.x, item.y);
    ...
    /** @param {CanvasRenderingContext2D} ctx */
    drawTraps(ctx, mode, liveTraps, mapDef, drawGlow, glowAlpha) {
    try {
      ctx.save();
      const itemsToDraw = (mode === 'play' || mode === 'race')
      ...
      for (const item of itemsToDraw) {
        if (item.consumed && (mode === 'play' || mode === 'race')) continue;
                ...
                // Compact warning field
        const warningPulse = Math.sin(currentTime * 0.008) * 0.5 + 0.5;
        
        // Small warning glow
        try {
          drawGlow(ctx, item.x, item.y, 16 * (0.8 + warningPulse * 0.4), '#F44336', glowAlpha(0.5 * warningPulse, 1.0 * warningPulse, 0.3));
        } catch {}
        
        // Compact warning flashes (2 rings only)
        if (warningPulse > 0.7) {
          ctx.strokeStyle = `rgba(244, 67, 54, ${warningPulse})`;
          ctx.lineWidth = 2;
          ctx.beginPath();
          ctx.arc(item.x, item.y, 12 + 6, 0, Math.PI * 2);
          ctx.stroke();
        }
        ...
        ctx.fillStyle = 'white';
        if (mode === 'editor' && mapDef?.editorVectorIconsOnly) {
          try { this._drawVectorIcon(ctx, 'trap', item.x, item.y, 12); } catch {}
        } else {
          const configEmoji = window.config?.powerUps?.trap?.emoji;
          const icon = (mode === 'editor' && mapDef?.editorEmojiEnabled === false) 
            ? 'TRP' 
            : (configEmoji || '🪤');
          ctx.fillText(icon, item.x, item.y);
    ...
    drawRams(ctx, mode, liveRams, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        // Fix: In editor mode, render both static rams from mapDef and live rams
        ...
        for (const r of ramsToDraw) {
          ...
          ctx.font = `bold ${Math.max(16, Math.round(r.r * 0.9))}px "Segoe UI Emoji","Segoe UI Symbol",system-ui,sans-serif`;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ...
          ctx.fillText('💥', r.x + shadowOffset, r.y + shadowOffset);
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const rGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'R' : '💥';
    ...
    drawShields(ctx, mode, liveShields, mapDef, drawGlow, glowAlpha) {
      try {
        ctx.save();
        // Fix: In play mode, render both static shields from mapDef and live shields
        const shieldsToDraw = (mode === 'play' || mode === 'race')
          ? [ ...(mapDef?.shields || []), ...(liveShields || []) ]
          : [ ...(mapDef?.shields || []), ...((Array.isArray(liveShields) ? liveShields : [])) ];
        ...
        for (const s of shieldsToDraw) {
          ...
          ctx.font = `bold ${Math.max(12, Math.round(s.r))}px "Segoe UI Emoji","Segoe UI Symbol",system-ui,sans-serif`;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ...
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const sGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'S' : '🛡️';
    ...
    /** Vẽ Carrots theo đúng logic hiện tại */
    ...
    /** Gọi nhanh tất cả nhóm động đã tách */
    drawDynamicItems(ctx, mode, mapDef, sources) {
      try {
        ...
        this.drawTraps(ctx, mode, S.liveTraps, mapDef, dg, ga);
    ...
    /** Vẽ Spinners theo logic hiện có (bao gồm trail, flash, glints, editor handle) */
    drawSpinners(ctx, mode, mapDef, liveSpinners) {
      try {
        ctx.save();
      } catch {}
      try {
        // CORRECTED FIX: Render ONLY live spinners in play mode, both in editor mode
        let spinnersToDraw = [];
        if (mode === 'play' || mode === 'race') {
          // Play mode: ONLY render live spinners (the real rotating ones), ignore static mapDef.spinners
          spinnersToDraw = liveSpinners || [];
        } else {
          // Editor mode: render both mapDef and live for editing
          spinnersToDraw = [ ...(mapDef?.spinners || []), ...((Array.isArray(liveSpinners) ? liveSpinners : [])) ];
        }
        ...
        for (const s of spinnersToDraw) {
          ...
          try {
            ...
            // Enhanced Spinner Design (matches extracted-inline.js)
            const baseColor = s.color || '#9E9E9E';
            const time = performance.now() * 0.001;
            const speed = s.speed || 1;
            const cornerRadius = Math.min(s.w, s.h) * 0.15;
            
            // Drop shadow for depth
            ctx.save();
            ctx.fillStyle = 'rgba(0, 0, 0, 0.3)';
            ctx.beginPath();
            ctx.roundRect(-s.w/2 + 2, -s.h/2 + 2, s.w, s.h, cornerRadius);
            ctx.fill();
            ctx.restore();
            ...
            ctx.fillStyle = baseColor;
            ctx.beginPath();
            ctx.roundRect(-s.w/2, -s.h/2, s.w, s.h, cornerRadius);
            ctx.fill();
            
            // No animated effects - keep it simple and clean
            
            // Glossy top highlight
            const topGrad = ctx.createLinearGradient(0, -s.h/2, 0, 0);
            topGrad.addColorStop(0, 'rgba(255, 255, 255, 0.4)');
            topGrad.addColorStop(1, 'rgba(255, 255, 255, 0)');
            
            ctx.fillStyle = topGrad;
            ctx.beginPath();
            ctx.roundRect(-s.w/2, -s.h/2, s.w, s.h/2, [cornerRadius, cornerRadius, 0, 0]);
            ctx.fill();
    ...
    /** Hiển thị Slipstream wakes (visuals) */
    ...
    /** Vẽ mũi tên băng chuyền (belt arrows) */
    ...
    /**
     * Vẽ toàn bộ khung hình theo thứ tự hiện tại. Không đổi hành vi.
     * env (tùy chọn) có thể cung cấp: { mode, mapDef, horses, liveBoosts, liveTurbos, liveTeleports, liveMagnets, liveTimeFreezes, liveGhosts, liveTraps, liveRams, liveShields, liveSpinners, particles, drawGlow, glowAlpha, carrotSpriteImg, drawVectorCarrot, canvas }
     */
    drawFrame(ctx, canvas, env) {
      ...
      try {
        this.drawDynamicItems(ctx, mode, mapDef, {
          ...
          liveTraps: ST.liveTraps || w.liveTraps,
    ...
    drawTornados(ctx, mode, liveTornados, mapDef, drawGlow, glowAlpha) {
      try {
        ctx.save();
        // Fix: In editor mode, render both static tornados from mapDef and live tornados
        ...
        for (const item of tornadosToDraw) {
          ...
          // === RenderModule: Advanced Visual Effects for Enhanced Game Experience ===
...
window.RenderModule = {
  ...
  darkenColor,
          ...
          // Tornado icon with vector support
          if (mode === 'editor' && w.editorVectorIconsOnly) { 
            try { this._drawVectorIcon(ctx, 'tornado', x, y, dynamicRadius); } catch {} 
          }
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const tornadoGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'T' : '🌪️';
            ctx.font = `bold ${Math.max(16, Math.round(dynamicRadius * 0.8))}px "Segoe UI Emoji"`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillStyle = 'rgba(0, 0, 0, 0.9)';
            ctx.fillText(tornadoGlyph, x + 1, y + 1);
            ctx.fillStyle = 'rgba(255, 255, 255, 1.0)';
            ctx.fillText(tornadoGlyph, x, y);
          }
    ...
    drawVolcanos(ctx, mode, liveVolcanos, mapDef, drawGlow, glowAlpha) {
      try {
        ctx.save();
        // Fix: In editor mode, render both static volcanos from mapDef and live volcanos
        ...
        for (const item of volcanosToDraw) {
          ...
          // Volcano icon with vector support
          if (mode === 'editor' && w.editorVectorIconsOnly) { 
            try { this._drawVectorIcon(ctx, 'volcano', x, y, dynamicRadius); } catch {} 
          }
          if (!(mode === 'editor' && w.editorVectorIconsOnly)) {
            const volcanoGlyph = (typeof w.mode !== 'undefined' && w.mode === 'editor' && !w.editorEmojiEnabled) ? 'V' : '🌋';
            ctx.font = `bold ${Math.max(16, Math.round(dynamicRadius * 0.8))}px "Segoe UI Emoji"`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillStyle = 'rgba(0, 0, 0, 0.9)';
            ctx.fillText(volcanoGlyph, x + 1, y + 1);
            ctx.fillStyle = 'rgba(255, 255, 255, 1.0)';
            ctx.fillText(volcanoGlyph, x, y);
          }
    ...
    drawLightnings(ctx, mode, liveLightnings, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        for (const item of lightningsToDraw) {
          ...
          ctx.font = `bold ${Math.max(16, Math.round(dynamicRadius * iconScale))}px "Segoe UI Emoji","Segoe UI Symbol",system-ui,sans-serif`;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          
          // Dynamic shadows
          ctx.fillStyle = `rgba(0, 0, 0, ${0.7 + pulse * 0.2})`;
          ctx.fillText('⭐', x + 2, y + 2); // Shadow
          ctx.fillStyle = `rgba(0, 0, 0, ${0.5 + pulse * 0.2})`;
          ctx.fillText('⭐', x + 1, y + 1); // Lighter shadow
          ctx.fillStyle = 'rgba(255, 255, 255, 1.0)';
    ...
    drawWarpzones(ctx, mode, liveWarpzones, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        // Combine static and live warpzones (critical for play mode)
        ...
        for (const item of warpzonesToDraw) {
          ...
          // Compact shield-like glow effect
          try {
            // Outer glow with dynamic size
            drawGlow(ctx, item.x, item.y, animatedRadius * 1.4, '#9C27B0', glowAlpha(0.2 * fastPulse, 0.4 * fastPulse, 0.1));
            // Mid glow
            drawGlow(ctx, item.x, item.y, animatedRadius * 1.2, '#E91E63', glowAlpha(0.4 * slowPulse, 0.6 * slowPulse, 0.15));
            // Inner intense glow
          ...
          ctx.font = `bold ${Math.max(12, Math.round(animatedRadius * 0.8))}px "Segoe UI Emoji","Segoe UI Symbol",system-ui,sans-serif`;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ...
          if (!(mode === 'editor' && window.editorVectorIconsOnly)) {
            const glyph = (mode === 'editor' && !window.editorEmojiEnabled) ? 'W' : '🌌';
    ...
    drawQuantumdashs(ctx, mode, liveQuantumdashs, mapDef, drawGlow, glowAlpha) {
      try {
        ...
        // Combine static and live quantum dashes (critical for play mode)
        ...
        for (const item of quantumdashsToDraw) {
          ...
          try {
            ...
            drawGlow(ctx, item.x, item.y, quantumRadius * 1.5, '#00BCD4', glowAlpha(0.15 * quantumWave, 0.3 * quantumWave, 0.08));
            // Energy wave layer
            drawGlow(ctx, item.x, item.y, quantumRadius * 1.3, '#4FC3F7', glowAlpha(0.3 * energyPulse, 0.5 * energyPulse, 0.12));
            // Core quantum field
          ...
          ctx.font = `bold ${Math.max(12, Math.round(quantumRadius * 0.8))}px "Segoe UI Emoji","Segoe UI Symbol",system-ui,sans-serif`;
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ...
          if (!(mode === 'editor' && window.editorVectorIconsOnly)) {
            const glyph = (mode === 'editor' && !window.editorEmojiEnabled) ? 'Q' : '🔮';
